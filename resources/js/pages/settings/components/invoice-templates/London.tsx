...
export function London({ invoice, color, showQr, invoiceUrl, footerTitle, footerNotes, remainingAmount, formatAmount, t, invoiceLogo, companyLogo }: any) {
            ...
            <tbody>
              <tr>
                <td style={{ verticalAlign: 'top' }}>
                  <h3 style={{ textTransform: 'uppercase', fontSize: '30px', fontWeight: 'bold', margin: '0 0 10px 0' }}>{translate('INVOICE')}</h3>
                ...
                <td style={{ verticalAlign: 'top', textAlign: 'right', paddingTop: '60px' }}>
                  {showQr && (
                    <div style={{ maxWidth: '114px', marginLeft: 'auto', marginBottom: '10px' }}>
                      <QRCodeGenerator value={invoiceUrl || 'https://example.com/invoice'} size={114} />
                    </div>
                  )}
                  <div>{translate('Number')}: {invoiceData.invoice_number}</div>
                  <div>{translate('Invoice Date')}: {new Date(invoiceData.invoice_date).toLocaleDateString()}</div>
                  <div>{translate('Due Date')}: {new Date(invoiceData.due_date).toLocaleDateString()}</div>
                </td>
              </tr>
            </tbody>
          </table>
