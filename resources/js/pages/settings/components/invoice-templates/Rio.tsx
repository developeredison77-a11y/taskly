...
export function Rio({ invoice, color, showQr, invoiceUrl, footerTitle, footerNotes, remainingAmount, formatAmount, t, invoiceLogo, companyLogo }: any) {
      ...
      <div style={{ width: '100%', margin: '0 auto', background: '#ffffff', boxShadow: '0 0 10px #ddd' }}>
        <div style={{ padding: '15px 30px', position: 'relative' }}>
          {(invoiceLogo || companyLogo || logoDark) && (
            <img src={invoiceLogo || companyLogo || logoDark} alt="Logo" style={{ position: 'absolute', top: '0', right: '0', maxWidth: '150px', maxHeight: '150px', objectFit: 'contain' }} />
          )}
          <div>
            <h3 style={{ textTransform: 'uppercase', fontSize: '20px', fontWeight: 'bold', margin: 0 }}>{translate('INVOICE')}</h3>
            <div style={{ maxWidth: '114px', maxHeight: '114px', marginTop: '15px', background: '#ffffff' }}>
              {showQr && <QRCodeGenerator value={invoiceUrl || 'https://example.com/invoice'} size={114} />}
            </div>
          </div>
        </div>
          ...
          <table style={{ width: '100%' }}>
            <tbody>
              ...
              <tr>
                ...
                <td style={{ padding: 0, textAlign: 'right' }}>{translate('Invoice Date')}: {new Date(invoiceData.invoice_date).toLocaleDateString()}<br />{translate('Due Date')}: {new Date(invoiceData.due_date).toLocaleDateString()}</td>
              </tr>
            </tbody>
          </table>
        </div>
            ...
            <tbody>
              <tr>
                ...
                <td style={{ verticalAlign: 'top', textAlign: 'right' }}>
                  <strong style={{ marginBottom: '10px', display: 'block' }}>{translate('Company')}:</strong>
                  <p style={{ margin: 0, lineHeight: '1.5' }}>
