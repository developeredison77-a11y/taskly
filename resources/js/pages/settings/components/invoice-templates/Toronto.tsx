...
export function Toronto({ invoice, color, showQr, invoiceUrl, footerTitle, footerNotes, remainingAmount, formatAmount, t, invoiceLogo, companyLogo }: any) {
        ...
        <div style={{ padding: '15px 30px' }}>
              ...
              <tr>
                <td style={{ padding: '15px 30px', verticalAlign: 'top' }}>
                  {(invoiceLogo || companyLogo || logoDark) && (
                    <img src={invoiceLogo || companyLogo || logoDark} style={{ maxWidth: '150px', maxHeight: '150px' }} alt="Logo" />
                  )}
                </td>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <tbody>
              <tr>
                <td style={{ padding: '15px 30px', verticalAlign: 'top' }}>
                  <h3 style={{ textTransform: 'uppercase', fontSize: '25px', fontWeight: 'bold', marginBottom: '15px' }}>{translate('INVOICE')}</h3>
                  <table style={{ width: '100%' }}>
                    <tbody>
                      <tr>
                        <td style={{ padding: 0 }}>{translate('Number')}: {invoiceData.invoice_number}</td>
                      </tr>
                      <tr>
                        <td style={{ padding: 0 }}>{translate('Invoice Date')}: {new Date(invoiceData.invoice_date).toLocaleDateString()}<br />{translate('Due Date')}: {new Date(invoiceData.due_date).toLocaleDateString()}</td>
                      </tr>
                    </tbody>
                  </table>
                </td>
