...
class UserController extends BaseController
{
     ...
     */
    public function destroyLoginHistory($loginHistoryOrId)
    {
        $authUser = Auth::user();

        // Handle both model binding and direct ID
        if ($loginHistoryOrId instanceof LoginHistory) {
            $loginHistory = $loginHistoryOrId;
        } else {
            $loginHistory = LoginHistory::find($loginHistoryOrId);
            if (!$loginHistory) {
                return redirect()->back()->with('error', __('Login history not found'));
            }
        }

        // Check permissions - only superadmin or the creator can delete
        if ($authUser->type !== 'superadmin' && $loginHistory->created_by !== $authUser->id) {
            return redirect()->back()->with('error', __('Unauthorized Access Prevented'));
        }
        ...
        try {
            ...
            return redirect()->back()->with('error', __('Unable to delete login history: ') . $e->getMessage());
        }
    }
